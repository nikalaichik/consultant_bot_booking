...
PROCEDURE_DESCRIPTIONS = {
    "cleaning": """
🧼 <b>Чистка лица</b> — это базовая гигиеническая процедура, необходимая для здоровья и красоты кожи. Она помогает глубоко очистить поры, удалить омертвевшие клетки и излишки кожного сала.

✨ <b>Эффект от процедуры:</b>
• Чистая, гладкая и сияющая кожа.
• Сужение пор и выравнивание тона лица.
• Уменьшение количества черных точек (комедонов) и воспалений.
• Улучшение проникновения активных компонентов из вашего домашнего ухода.

🚫 <b>Основные противопоказания:</b>
• Обострение кожных заболеваний (дерматит, экзема).
• Герпес в активной стадии.
• Множественные гнойничковые высыпания.
• Повышенная температура тела.
""",
    "carboxy": """
💨 <b>Неинвазивная карбокситерапия</b> — это процедура насыщения кожи углекислым газом (CO2) без уколов. В ответ на воздействие CO2, организм усиливает кровообращение и выработку коллагена, что приводит к видимому омоложению.

✨ <b>Эффект от процедуры:</b>
• Мгновенный лифтинг-эффект и повышение упругости кожи.
• Уменьшение отечности и темных кругов под глазами.
• Выравнивание тона и здоровый цвет лица.
• Снятие воспалений и улучшение состояния кожи при акне.
...
""",
    "microneedling": """
🎯 <b>Микронидлинг (фракционная мезотерапия)</b> — это современная методика омоложения с помощью аппарата Dermapen. Множество микропроколов стимулируют кожу к самообновлению и выработке собственного коллагена и эластина.

✨ <b>Эффект от процедуры:</b>
• Заметное сокращение морщин и повышение плотности кожи.
• Уменьшение рубцов, шрамов и следов постакне.
• Сужение пор и выравнивание рельефа кожи.
• Улучшение цвета лица и борьба с пигментацией.
...
""",
    "peeling": """
    <b>Пилинг</b> — это процедура щадящего очищения кожи, лечения акне, выравнивания тона кожи. Пилинг преставляет собой удаление верхних слоев кожи, что позволяет добиться эффекта омоложения кожи, восстановить защитные свойства кожи и решить проблемы эстетического характера.

✨ <b>Эффект от процедуры:</b>
• Осветление пигментных пятен и веснушек.
• Выравнивание тона и придание коже сияния.
• Уменьшение поверхностных морщин и повышение тонуса.
• Борьба с высыпаниями, устранение постакне.

🚫 <b>Основные противопоказания:</b>
• Аллергия на компоненты препарата.
• Свежий загар или повреждения кожи.
• Герпес в активной стадии.
• Прием препаратов, повышающих фоточувствительность.
""",
    "mesopeel": """
🔄 <b>Мезопилинг</b> — это процедура, сочетающая в себе преимущества химического пилинга (отшелушивание) и мезотерапии (введение активных веществ). Она мягко обновляет кожу, одновременно насыщая ее витаминами и аминокислотами.

✨ <b>Эффект от процедуры:</b>
• Осветление пигментных пятен и веснушек.
• Выравнивание тона и придание коже сияния.
• Уменьшение поверхностных морщин и повышение тонуса.
• Борьба с высыпаниями и жирным блеском.
...
""",
    "massage": """
👐 <b>Массаж лица</b> — это не только приятная, но и очень эффективная процедура для поддержания молодости и тонуса кожи. Профессиональный массаж снимает мышечное напряжение, улучшает кровообращение и лимфодренаж.

✨ <b>Эффект от процедуры:</b>
• Уменьшение отечности и формирование четкого овала лица.
• Разглаживание мелких морщин и носогубных складок.
• Улучшение цвета лица за счет активизации кровообращения.
• Глубокое расслабление и снятие стресса.

🚫 <b>Основные противопоказания:</b>
• Наличие гнойничковых воспалений на коже (акне).
• Обильные высыпания, дерматит в стадии обострения.
• Нарушение целостности кожи.
• ЛОР-заболевания в острой стадии (гайморит).
""",
    "consultation": """
👩‍⚕️ <b>Консультация косметолога</b> — это первый и самый важный шаг на пути к здоровой и красивой коже. На консультации специалист проведет диагностику, определит ваш тип и текущее состояние кожи.

✨ <b>Что вы получите:</b>
• Профессиональную оценку состояния вашей кожи.
• Ответы на все волнующие вас вопросы.
• Индивидуальный план процедур, подходящий именно вам.
• Рекомендации по правильному домашнему уходу.
"""
}
...
async def booking_start_handler(callback: types.CallbackQuery, state: FSMContext, bot_logic: SimpleBotLogic):
    ...
    procedure_key = callback.data.replace("book_", "")

    # Словарь для сопоставления ключа с полным названием
    procedure_names = {
    ...
    # Получаем готовый текст из словаря PROCEDURE_DESCRIPTIONS
    info_response = PROCEDURE_DESCRIPTIONS.get(procedure_key, "К сожалению, описание для этой процедуры не найдено.")
    procedure_name = procedure_names.get(procedure_key, "выбранную процедуру")

    # Сохраняем данные в FSM
    await state.update_data(procedure=procedure_key, procedure_name=procedure_name)

    # Формируем итоговое сообщение
    confirmation_text = f"Вы выбрали запись на: <b>{procedure_name}</b>\n{info_response}\n\n"
    confirmation_text += "Хотите продолжить и выбрать время?"
    ...
    procedure_queries = {
        "cleaning": "основная информация о чистке лица, показания, противопоказания, эффект",
        "carboxy": "что такое карбокситерапия, эффекты, показания, курс",
        "microneedling": "что такое микронидлинг, курс, эффект, противопоказания",
        "massage": "информация о массаже лица, виды, эффекты, противопоказания",
        "peeling": "информация о пилингах которые решаюют проблемы клиента и подходят ему, показания, эффекты, курс, противопоказания",
