...
async def booking_start_handler(callback: types.CallbackQuery, state: FSMContext, bot_logic: SimpleBotLogic):
    ...
    confirmation_text = f"Вы выбрали запись на: **{procedure_name}**\n\n"
    confirmation_text += "🔍 **Краткая информация о процедуре:**\n"
...
async def booking_confirmation_handler(callback: types.CallbackQuery, state: FSMContext, bot_logic: SimpleBotLogic):
    ...
    try:
        ...
        text = f"📅 **Выберите удобное время для записи на {procedure_name}:**\n\n"
...
async def time_slot_selected_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        contact_text = f"""✅ **Выбрано время:** {selected_slot.display}
...
async def contact_info_handler(message: types.Message, state: FSMContext):
    ...
    confirmation_text = f"""📋 **ПРОВЕРЬТЕ ДАННЫЕ ЗАПИСИ:**

    🎯 **Процедура:** {procedure_name}
    📅 **Дата и время:** {selected_slot_data['display']}
    👤 **Контактные данные:**
    {message.text}

    ⚠️ **Внимание:** После подтверждения время будет забронировано в календаре косметолога.
...
async def final_booking_confirmation_handler(callback: types.CallbackQuery, state: FSMContext,
    ...
    try:
        ...
        calendar_status = "🗓️ **Запись добавлена в календарь косметолога!**" if calendar_event_id else "📞 **Администратор подтвердит время записи.**"
...
{calendar_status}
    ...
    ⚠️ **ВАЖНО:** Приходите за 10 минут до начала процедуры!"""
    ...
    {calendar_info}"""
        ...
        error_text = f"""😔 **ОШИБКА ПРИ СОЗДАНИИ ЗАПИСИ**
...
async def change_time_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = f"📅 **Выберите другое время для записи на {procedure_name}:**\n\n⏰ Доступные дата и время:"
